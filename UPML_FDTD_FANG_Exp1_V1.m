...
grid on;
% 这个情况非常典型！在我提供代码的基础上出现"长时间后范数爆炸"，99% 的可能性不是数学理论推导有误，而是两个工程实现上的细节漏洞：
% 
% 恒定 
% σ=4 在仅有 5 层 PML 时阻抗严重失配，导致产生巨大的数值反射，而在 Yee 格式的弱稳定性下，反射波会持续累积能量，最终爆炸（论文引言中明确指出 UPML 的 Yee 格式是"弱稳定"的，对高频噪声很敏感）。
% 散射体边界（矩形导体）处的磁场更新存在"半格点"错位，在强反射波冲击下，边界附近会产生虚假的高频振荡，进而触发不稳定性。
% 为了解决这个问题，我将为你提供修正版的第 5 步完整循环。核心改动有三处：
% 
% 改用渐变 PML（
% σ
% σ 从界面到外边界二次方增加），极大减少反射；
% 增加 PML 层数（从 5 层增加到 10 层），让吸收更充分；
% 修正散射体角点附近的磁场更新逻辑，避免出现未定义的差分项。